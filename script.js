document.addEventListener("DOMContentLoaded", () => {
  ...
  botoes.forEach(botao => {
    botao.addEventListener("click", () => {
      ...
      const idAba = botao.id.replace("aba-", "");
  ...
  function atualizarPericia(id) {
    ...
    if (!input) return;
    document.getElementById(id + "_half").textContent = Math.floor((parseInt(input.value) || 0) / 2);
    document.getElementById(id + "_fifth").textContent = Math.floor((parseInt(input.value) || 0) / 5);
  }
  ...
  function criarPericia(p, container, prefixo = "") {
    ...
    if (!p.auto) input.addEventListener("input", () => atualizarPericia(id));

    if (p.auto === "des") {
      const desInput = document.getElementById("destreza");
      const atualizar = () => { input.value = Math.floor((parseInt(desInput?.value) || 0)/2); atualizarPericia(id); };
      atualizar();
    ...
    if (p.auto === "edu") {
      ...
      const atualizar = () => { input.value = parseInt(eduInput?.value) || 0; atualizarPericia(id); };
      atualizar();
  ...
  function rolarAtributo(id) {
    let valor;
    if (["tamanho","inteligencia","educacao"].includes(id)) valor = (d6()+d6()+6)*5;
    else valor = (d6()+d6()+d6())*5;
    const input = document.getElementById(id);
  ...
  function rolarDado(tipo){
    let resultado;
    switch(tipo){
      case "D4": resultado=Math.floor(Math.random()*4)+1; break;
      case "D6": resultado=Math.floor(Math.random()*6)+1; break;
      case "D8": resultado=Math.floor(Math.random()*8)+1; break;
      case "D10": resultado=Math.floor(Math.random()*10)+1; break;
      case "D12": resultado=Math.floor(Math.random()*12)+1; break;
      case "D20": resultado=Math.floor(Math.random()*20)+1; break;
      case "D100": resultado=Math.floor(Math.random()*100)+1; break;
    }
  ...
  function atualizarSecundarios() {
    ...
    if (document.getElementById("vida")) document.getElementById("vida").value = hp;
  ...
  ["forca","constituicao","tamanho","destreza","idade"].forEach(id => {
